...
class XMPPController: NSObject, XMPPStreamDelegate, XMPPRosterDelegate, XMPPvCardTempModuleDelegate {
    ...
    func connect() {
        ...
        if(userJID == "") {
            self.userJID = self.defaults.string(forKey: "savedUserJID")!
            self.password = self.defaults.string(forKey: "savedPassword")!
        }
    ...
    func sendSubscriptionRequest(to jid: XMPPJID, xmppStream: XMPPStream) {
        ...
        KeyManager.shared.encryptAESKey(aesKey: aesKey, withRSAKey: jid.user!) { encryptedAESKey in
            ...
            let base64EncodedAESKey = encryptedAESKey.base64EncodedString()
            ...
            friendRequest.friends = true
            friendRequest.base64EncodedEncryptedAESKey = base64EncodedAESKey
    ...
    func sendMessage(message: String, recipient: String) {
        ...
        if let user = user {
            if let encryptedData = KeyManager.shared.AESencryptMessage(username: user, message: message) {
    ...
    func xmppStream(_ sender: XMPPStream, didReceive message: XMPPMessage) {
        ...
        if let user = self.xmppStream.myJID?.user {
            let keyTag = "\ncom.cipher.\(user).privateKey\n"
            ...
            let messageBody = KeyManager.shared.decryptMessageWithAESAndRSA(privateKeyTag: keyTag, messageData: cipherMessage, sender: sender! )
            print("\nMessage Decrypted!\n")
    ...
    func xmppRoster(_ sender: XMPPRoster, didReceivePresenceSubscriptionRequest presence: XMPPPresence) {
        ...
        print("No")
         ...
         let encryptedAESKeyData = combinedData.subdata(in: 0..<16)
         let ivData = combinedData.subdata(in: 16..<(encryptedAESKeyIV!.count))

         // Save the friend request to CoreData
         let friendRequest = FriendRequest(context: self.context)
         friendRequest.username = username
         friendRequest.friends = false
         friendRequest.base64EncodedEncryptedAESKey = encryptedAESKeyData.base64EncodedString()
