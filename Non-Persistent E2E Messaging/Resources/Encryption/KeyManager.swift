...
class KeyManager{
    ...
    func AESencryptMessage(message: String, key: Data, iv: Data) -> Data? {
        guard let messageData = message.data(using: .utf8) else {
            print("Error converting message to Data")
            return nil
        }
        
        let encryptedData = messageData.aesEncrypt(key: key, iv: iv)
    ...
    func AESencryptMessage(username: String, message: String) -> Data? {
        ...
        do {
            ...
            if let friendRequest = fetchedRequests.first {
                if let base64EncodedEncryptedAESKey = friendRequest.base64EncodedEncryptedAESKey,
                   let keyData = Data(base64Encoded: base64EncodedEncryptedAESKey),
    ...
    /*
    func decryptMessage(encryptedData: Data, privateKeyTag: String) -> String? {
    ...
    func decryptMessageWithAESAndRSA(privateKeyTag: String, messageData: Data, sender: String) -> String? {

        // Step 2: Retrieve private key from keychain
        let query: [String: Any] = [
            kSecClass as String: kSecClassKey,
            kSecAttrApplicationTag as String: privateKeyTag.data(using: .utf8)!,
            kSecAttrKeyType as String: kSecAttrKeyTypeRSA,
        ...
        var base64EncodedEncryptedAESKey: String?
        var base64IV: String?
        ...
        do{
            ...
            for reqs in friendReqs {
                if  reqs.username == sender {
                    base64EncodedEncryptedAESKey = reqs.base64EncodedEncryptedAESKey
                    base64IV = reqs.iv
                }
            }
        }
        ...
        let encryptedAESKeyData = Data(base64Encoded: base64EncodedEncryptedAESKey!)
        
        let ivData = Data(base64Encoded: base64IV!)
        
        let decryptedAESKeyData = SecKeyCreateDecryptedData(privateKey, algorithm, encryptedAESKeyData! as CFData, &error)
        
        
        
        if let error = error {
            print("Error decrypting AES key: \(error.takeRetainedValue())")
            return nil
        }

        // Step 5: Decrypt the message using the decrypted AES key and IV
        guard let decryptedAESKey = decryptedAESKeyData as Data?,
              let decryptedMessage = messageData.aesDecrypt(key: decryptedAESKey, iv: ivData!) else {
